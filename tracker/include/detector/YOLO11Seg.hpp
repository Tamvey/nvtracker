...
public:
  YOLOv11SegDetector(const std::string modelPath, const std::string labelsPath,
                     bool useGPU = false);
...
inline void
YOLOv11SegDetector::drawSegmentations(cv::Mat &image,
                                      const std::vector<Segmentation> &results,
                                      float maskAlpha) const {
  for (const auto &seg : results) {
    if (seg.conf < CONFIDENCE_THRESHOLD) {
...
inline std::vector<Segmentation>
